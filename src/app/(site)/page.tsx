...
interface ApiProduct { id: string; title: string; slug: string; price: number; oldPrice?: number; coverImage?: string; featured: boolean; category?: { name: string }; }
interface ApiPost { id: string; title: string; slug: string; category?: { name: string }; publishedAt?: string; createdAt: string; }
...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("/api/products?status=active&featured=true")
      .then(r => r.json())
      .then(d => setProducts((d.products || []).slice(0, 3)))
      .catch(() => {});
            ...
            {products.length > 0 ? products.map((p) => {
              ...
              [...Array(3)].map((_, i) => (
                <div key={i} className="glass-card overflow-hidden animate-pulse">
                  <div className="aspect-video" style={{ background: "rgba(20,20,32,0.5)" }} />
                  <div className="p-5 space-y-3">
                    ...
                    <div className="h-3 rounded w-1/2" style={{ background: "rgba(30,30,50,0.8)" }} />
                  </div>
                </div>
              ))
            )}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            {posts.length > 0 ? posts.map((post) => (
              ...
              [...Array(3)].map((_, i) => (
                <div key={i} className="p-4 rounded-2xl animate-pulse flex items-center gap-4" style={{ background: "rgba(10,10,16,0.7)", border: "1px solid rgba(255,255,255,0.04)" }}>
                  <div className="w-9 h-9 rounded-xl flex-shrink-0" style={{ background: "rgba(30,30,50,0.8)" }} />
                  <div className="flex-1 space-y-2">
                    <div className="h-3.5 rounded w-3/4" style={{ background: "rgba(30,30,50,0.8)" }} />
                    <div className="h-2.5 rounded w-1/3" style={{ background: "rgba(30,30,50,0.8)" }} />
                  </div>
                </div>
              ))
            )}
